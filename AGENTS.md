...
- Google Drive/외부 공유용 stable view 생성은 `$kb-publish` skill prompt를 우선 사용한다.
- deterministic script는 문서 형식, 변환, 검사, 색인, 리포트, 배포 사본 생성만 수행하며 기획/기술 결정을 자동 확정하지 않는다.

## 9. 결정적 검사 규칙

문서, 템플릿, 프롬프트, 폴더 구조를 생성하거나 수정한 경우 가능하면 `$kb-quality-checks` skill prompt로 결정적 검사를 요청한다.
