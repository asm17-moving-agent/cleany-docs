...
Mission Manager는 지정 구역 정리 mission의 전체 흐름을 FSM으로 관리한다.
...
`skip`/`human_review` task는 `PLAN_TASKS`를 조기 종료시키지 않는다. `plan()`이 `OK`를 반환하면 `EXECUTE_TASKS`로 정상 진행하고, `skipped_tasks`/`needs_human_review`는 `REPORT`에서 최종 `status`를 정할 때 반영된다 (자세한 내용은 [Report](#9-report) 참고).
...
MVP mission request는 지정 구역 정리 작업을 시작하기 위한 최소 정보만 포함한다.
...
FSM core와 ROS 2 transport를 분리한다. 안정된 최소 wire contract는 `cleany_interfaces`의 다음 파일로 표현한다.
...
ROS의 `ModuleResult`에는 공통 envelope만 둔다.
...
Python core의 `ModuleResult.data: Any`를 그대로 ROS message에 넣지 않는다. Navigator의 `current_pose`, Perception의 `objects`, Planner의 `task_plan`, Skill Executor의 검증 결과는 각 action/service가 구체적인 typed payload로 정의한다. `TaskPlan`, `Skill`, `DetectedObject`, `WorldState`의 정확한 schema는 아직 미해결이므로 별도 검토 전까지 interface package에 고정하지 않는다.

ROS message에는 Python의 `None`이 없으므로 성공 시 `failure_code`, 실패 지점이 없을 때 `failed_task`는 빈 문자열로 표현한다. ROS wrapper는 빈 문자열과 core의 `None`을 양방향으로 변환한다.
