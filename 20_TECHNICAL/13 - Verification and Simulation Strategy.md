...
| 단계 | 검증 목표 | 통과 기준 |
|---|---|---|
| Core | Mission·Planner·Guard 순수 logic | 성공·실패·차단·부분 결과 경로가 재현됨 |
...
| AI 통합 | ER 2 task order와 tool orchestration | 허용 tool만 실행되고 재관찰로 결과를 판단함 |
...
- collision, timeout, grasp 실패와 결과 반환
...
4. RuleBasedPlanner 또는 ER 2가 작업 순서를 제안한다.
5. Local Guard가 요청을 검증한다.
6. MuJoCo 또는 실제 Manipulation backend가 skill을 실행한다.
7. 작업 후 관찰로 실제 변화를 확인한다.
8. 복귀와 MissionReport를 완료한다.
...
- 작업 전후 관찰
- Planner proposal과 Guard 승인·거절 이유
