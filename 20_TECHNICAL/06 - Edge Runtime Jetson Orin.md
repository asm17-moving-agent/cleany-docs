...
flowchart TB
    subgraph cloud["Cloud · 운영자 영역"]
        ...
        er2["Gemini Robotics ER 2<br/>장면 추론·Task / Tool 제안"]
    end
    ...
    subgraph edge["Robot Edge · Jetson Orin NX"]
        ...
        ros["ROS 2 Runtime<br/>Nav2 · Perception · Planner<br/>Capability 물리 제약 검증·실행"]
    ...
    network <-->|ER 2 context·proposal| ros
    mission <--> ros
    sensors -->|관측·상태| ros
    ros --> logs
    ros -->|명령| controllers --> actuators
    actuators -->|상태·피드백| sensors
    estop -.->|cloud·일반 ROS와 독립된 차단| controllers
```
...
담당하지 않는다. ER 2 호출은 고정 주기가 아니라 작업 전 관찰과 high-level 행동의
완료·실패·장면 변화 checkpoint를 기준으로 한다.
