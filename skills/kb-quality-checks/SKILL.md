...
description: 끌리니 기획 KB의 결정적 검사를 실행한다. Markdown formatting, Obsidian YAML metadata, 필수 폴더/파일 구조, 내부 링크, Codex prompt 검사 규칙을 확인할 때 사용한다.
tags:
  - skill
  - quality-checks
  - validation
...
| Prompt 검사 | `scripts/check_prompts.py` | Codex prompt에 `$kb-ingest`, `$kb-quality-checks` 기반 workflow 안내 포함 여부 |
...
uv run python skills/kb-quality-checks/scripts/check_prompts.py .
```
