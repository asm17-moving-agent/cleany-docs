...
description: 끌리니 KB의 공유 가능한 무결성을 검사한다. 문서·metadata·폴더·repo skill을 바꾼 뒤 핵심 구조, YAML, 최소 상태값, 내부 참조와 skill 진입점을 확인할 때 사용한다.
---
...
| Metadata 검사 | `scripts/check_metadata.py` | 공식 문서의 `status`, 선택 metadata의 최소 타입과 허용값 |
| 참조 검사 | `scripts/check_references.py` | Markdown 링크·이미지, wiki link·embed, YAML 내부 경로 존재 |
| Skill 검사 | `scripts/check_skills.py` | 발견된 repo skill manifest와 Codex 진입점 일치 |

다음 항목은 CI 실패 조건으로 삼지 않는다.

- 핵심 계층 밖의 개별 문서·폴더 이름
- 공백, 줄바꿈, 표 정렬 같은 Markdown 스타일
