...
| 분실물 후보와 저신뢰 물체를 원위치 기록할지, 보관까지 수행할지, 사용자에게 어떻게 표시·알림할 것인가? | 현재 MVP 초안은 해당 물체를 건드리지 않는 방향이지만, 보관 경로도 범위 대안에 포함돼 있다. 결과 표시 방식도 정해지지 않았다. | [Target Scenario](<02 - Target Scenario.md>), [Scope and Non-Goals](<04 - Scope and Non-Goals.md>), [MVP 기능 범위](<../30_DECISIONS/Planning/260708 - MVP 기능 범위.md>) | 높음 | 추가 정의 필요 |
